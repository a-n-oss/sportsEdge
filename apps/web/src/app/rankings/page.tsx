...
export default async function RankingsPage({ searchParams }: RankingsPageProps) {
  ...
  const selected = params.league?.toLowerCase()
  const readyKeys = new Set(readyLeagueKeys(leagues))
  const targetLeagues = selected ? [selected] : [...readyKeys]
  const stubSelected = !isSelectedLeagueReady(selected, readyKeys)

  const standingsByLeague = await Promise.all(
    targetLeagues.map(async (league) => ({
      league,
      standings: await getStandings(league).catch(() => []),
    }))
  )
  ...
  return (
    <div className="space-y-8">
      <div>
        <PageHeader
          eyebrow={selected ? formatLeagueLabel(selected) : "Ready leagues"}
      ...
      {standingsByLeague.map(({ league, standings }) => (
        <section key={league} className="panel overflow-hidden">
          {!selected && (
          ...
          {standings.length === 0 ? (
            <p className="p-10 text-center text-muted-foreground">
              No rankings for {formatLeagueLabel(league)} yet.
            </p>
          ) : (
            standings.map((row) => (
              <Link
      ...
      {targetLeagues.length === 0 && (
        <p className="panel border-dashed p-10 text-center text-muted-foreground">
          No rankings for ready leagues yet.
        </p>
      )}
    </div>
  )
